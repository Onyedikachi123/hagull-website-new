...
// ─── Inline brand SVGs (lucide-react v1.14 has no social icons) ───────────────
const XIcon = () => (
                ...
                <ul className="space-y-2.5">
                  {links.map(({ label, href }) => (
                    <li key={`${label}-${href}`}>
                      <Link
                        href={href}
                        className="text-sm text-white/55 hover:text-white transition-colors duration-200 flex items-center gap-1 group"
                      >
                        {label}
                        <ArrowUpRight
                          size={12}
